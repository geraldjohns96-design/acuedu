document.addEventListener('DOMContentLoaded', () => {
        ...
        submitBtn.addEventListener('click', async (e) => {
            ...
            if (!submitBtn.disabled) {
                ...
                // NUEVO: Bucle que mantiene viva la interfaz si hay errores de selección
                let paso2Exitoso = false;
                let modalData = result1.data;

                while (!paso2Exitoso) {
                    const seleccion = await abrirModalRapido(modalData);
                    ...
                    if (!result2.success) {
                        if (result2.error === "SELECCION_INCORRECTA") {
                            // Ocultamos el loader para mostrar la alerta, y el bucle reiniciará el modal al hacer click en OK
                            document.getElementById('globalLoader').style.display = 'none';
                            alert('⚠️ Seleccionó mal la dirección o el suscriptor. Por favor, intente de nuevo seleccionando la opción correcta.');
                        } else {
                            throw new Error(result2.error); // Rompe el bucle si es un error distinto
                        }
                    } else {
                        paso2Exitoso = true; // Rompe el bucle y avanza
